<html>
<head>
    <title>BON DE COMMANDE DE FOURNISSEUR - <?=$commande->code_commande?></title>
    <style>
...
<header>
    <table style="width: 100%;">
        <tr>
            ...
            <td style="width: 35%; text-align: center;"><div class="title" style="width: 200px; margin-top: 60px; background-color: #cdcdcd; font-family: Arial, Helvetica, sans-serif;">BON DE COMMANDE <span style="font-size: 12px;">DE FOURNISSEUR</span></div></td>
        </tr>
    </table>
</header>
...
<main style="margin-right: 20px; margin-left: 20px;">
                ...
                <div style="width: 100%; border: 2px solid black; padding: 10px; font-size: 14px;">
                    <table style="width: 100%;">
                        <tr>
                            ...
                            <td style="font-weight: 600;"><?=$commande->code_commande?></td>
                        </tr>
                        <tr>
                            <td style="white-space: nowrap; width: 1px;">Date Commande</td>
                            <td style="white-space: nowrap; width: 1px;">&nbsp; : &nbsp;</td>
                            <td style="font-weight: 600;"><?=date("d/m/Y", strtotime($commande->date_commande))?></td>
                        </tr>
                    </table>
                </div>
            </td>
    ...
    <table class="table" style="width: 100%; margin-top: 20px;">
        ...
        foreach ($commande_details as $val):
            ...
            $tr_classe = $nbr < count($commande_details) ? "whiteBB" : "";
            $td_classe = "";
            if($nbr == 1 && $nbr == count($commande_details)) { $td_classe .= " pushFirst "; }
            else
            {
                if($nbr == 1) { $td_classe .= " pushTop "; }
                if($nbr == count($commande_details)) { $td_classe .= " pushBottom "; }
                if($nbr != 1 && $nbr != count($commande_details)) { $td_classe .= " pushNormal "; }
            }
            ?>
            <tr class="<?=$tr_classe?>">
