...
<header>
    <table style="width: 100%;">
        <tr>
            <td style="width: 45%;"><img src="<?=_STE_LOGO_?>" style="height: 160px; width: 250px; margin-top: 20px; margin-left: 40px;"></td>
...
<main style="margin-right: 20px; margin-left: 20px;">
                ...
                <div style="width: 100%; border: 2px solid black; padding: 10px; font-size: 14px;">
                    <table style="width: 100%;">
                        ...
                        <?php if(!empty($vente->reference)): ?>
                            <tr>
                                <td style="white-space: nowrap; width: 1px;">Référence</td>
                                <td style="white-space: nowrap; width: 1px;">&nbsp; : &nbsp;</td>
                                <td style="font-weight: 600;"><?=$vente->reference?></td>
                            </tr>
                        <?php endif; ?>
                        <tr>
    ...
    //         endif;
    //     endforeach; ?>
    <?php if(_ENABLE_SERVICE_ && $vente_details_service): ?>
    <table class="table" style="width: 100%; margin-top: 20px;">
        ...
        foreach ($vente_details_service as $val):
            ...
            $tr_classe = $nbr < count($vente_details_service) ? "whiteBB" : "";
            $td_classe = "";
            if($nbr == 1 && $nbr == count($vente_details_service)) { $td_classe .= " pushFirst "; }
            else
            {
                if($nbr == 1) { $td_classe .= " pushTop "; }
                if($nbr == count($vente_details_service)) { $td_classe .= " pushBottom "; }
                if($nbr != 1 && $nbr != count($vente_details_service)) { $td_classe .= " pushNormal "; }
            }
            ?>
            <tr class="<?=$tr_classe?>">
                <td class="<?=$td_classe?>" style="width: 5%;"><?=$nbr++?></td>
                <td class="<?=$td_classe?>" style="width: 42%;">
                    <?php
                ...
                <td class="<?=$td_classe?>" style="width: 10%;">
                    <?php
                    if(!empty($val["id_produit"])) {
                        echo $val["quantite"];
                    }
    ...
    <table class="table" style="width: 100%; margin-top: 20px;">
        ...
        foreach ($vente_details_produit as $val):
            ...
            $tr_classe = $nbr < count($vente_details_produit) ? "whiteBB" : "";
            $td_classe = "";
            if($nbr == 1 && $nbr == count($vente_details_produit)) { $td_classe .= " pushFirst "; }
            else
            {
                if($nbr == 1) { $td_classe .= " pushTop "; }
                if($nbr == count($vente_details_produit)) { $td_classe .= " pushBottom "; }
                if($nbr != 1 && $nbr != count($vente_details_produit)) { $td_classe .= " pushNormal "; }
            }
            ?>
            <tr class="<?=$tr_classe?>">
                <td class="<?=$td_classe?>" style="width: 5%;"><?=$nbr++?></td>
        ...
        <tr class="th">
            <td colspan="5"><strong style="font-size: 14px;">TVA à 20%</strong> &nbsp; <?=$vente->tva == 0 ? "<span style='float: right;'>( Exonération de la TVA )</span>" : ""?></td>
    ...
    <?php
    $exp = explode('.', number_format((float)$total_vente["total_vente"],2,".",""));
