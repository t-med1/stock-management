<div class="row">
    ...
    <form method="post" action="<?=base_url()?>index.php/avance/r_modifier">

        <input type="hidden" name="id_avance_retour" value="<?=$avance_retour->id_avance_retour?>">

        <div class="col-md-6">
            <div class="box box-info box-solid">
                <div class="box-header with-border">
                    <h3 class="box-title">Modifier Retour d'Avance</h3>
                ...
                <div class="box-body">
                    <div class="form-group">
                        ...
                        <input type="date" class="form-control" name="date_retour" value="<?=date("Y-m-d", strtotime($avance_retour->date_retour))?>" placeholder="Date Retour" required>
                    </div>
                    <div class="form-group">
                        ...
                        <select class="form-control select2" name="id_client" id="id_client" required>
                            <?php
                                ...
                                foreach ($clients as $val):
                                    if($val["id_client"] == $avance_retour->id_client) {
                                        $client_exists = true;
                                    }
                            ?>
                            ...
                            <?php if(!$client_exists): ?>
                                <option value="<?=$client->id_client?>"
                                        data-avance="<?=htmlspecialchars(json_encode(getAvanceTotal($client->id_client)))?>" >
                                    <?=$client->full_name?>
                                </option>
                            <?php endif; ?>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" step="any" min="0.1" class="form-control" name="montant" id="montant" value="<?=$avance_retour->montant?>" placeholder="Montant" required>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" name="description" placeholder="Description" maxlength="200" rows="3"><?=$avance_retour->description?></textarea>
                    </div>
                </div>
...
<script type="text/javascript">
    $(document).ready(function ()
    {
        ...
        $("#id_client").val(<?=$avance_retour->id_client?>).trigger("change");
        ...
        $("#retour_methode").on("change", function ()
        {
            if($(this).val() != undefined && $(this).val() != null)
            {
                if($(this).val() == 1)
                {
                    ...
                    $("#montant").attr("max", '<?=(getCaisseTotal() + ($avance_retour->methode == 1 ? $avance_retour->montant : 0))?>');
                }
                ...
                else if($(this).val() == 5)
                {
                    ...
                    {
                        $("#total_avance_1").html((avance.total_espece+<?=($avance_retour->methode==5 && $avance_retour->type_avance==1? $avance_retour->montant : 0)?>).toFixed(2));
                        $("#total_avance_2").html((avance.total_cheque+<?=($avance_retour->methode==5 && $avance_retour->type_avance==2? $avance_retour->montant : 0)?>).toFixed(2));
                        $("#total_avance_3").html((avance.total_virement+<?=($avance_retour->methode==5 && $avance_retour->type_avance==3? $avance_retour->montant : 0)?>).toFixed(2));
                    }
                    $("#avance_div").show();
                }
                else
        ...
        $("input[type=radio][name='type_avance']").on("change", function() {
            ...
            if(avance != undefined && avance != null && avance != "") {
                switch (this.value.toString()) {
                    case "1" : max = (avance.total_espece+<?=($avance_retour->methode==5 && $avance_retour->type_avance==1? $avance_retour->montant : 0)?>); break;
                    case "2" : max = (avance.total_cheque+<?=($avance_retour->methode==5 && $avance_retour->type_avance==2? $avance_retour->montant : 0)?>); break;
                    case "3" : max = (avance.total_virement+<?=($avance_retour->methode==5 && $avance_retour->type_avance==3? $avance_retour->montant : 0)?>); break;
                    default: max = 0;
                }
            }
        ...
        $("#retour_methode").val(<?=$avance_retour->methode?>).trigger("change");
        <?php
        if($avance_retour->methode == 2 || $avance_retour->methode == 3) :
            $cheque = getPaiementDetails($avance_retour->id_avance_retour, "avance_retour");
        ?>
        $("#cheque_montant").val("<?=$cheque->montant?>");
        $("#cheque_date").val("<?=$cheque->date_cheque?>");
        $("#cheque_reference").val("<?=$cheque->reference?>");
        <?php elseif ($avance_retour->methode == 5): ?>
        $("input[type=radio][name='type_avance']").each(function (i, el) {
            if(el.value.toString() == '<?=$avance_retour->type_avance?>') {
                el.checked = true;
            }
        });
        <?php endif; ?>
