<div class="row">
    ...
    <form method="post" action="<?=base_url()?>index.php/charge/modifier">

        <input type="hidden" name="id_charge" value="<?=$charge->id_charge?>">

        <div class="col-md-6">
            <div class="box box-default box-solid">
                <div class="box-header with-border">
                    <h3 class="box-title">Modifier Charge</h3>
                ...
                <div class="box-body">
                    <div class="form-group">
                        ...
                        <input type="date" class="form-control" name="date_charge" value="<?=date("Y-m-d", strtotime($charge->date_charge))?>" placeholder="Date Charge" required>
                    </div>
                    <div class="form-group">
                        <label><span class="text-red" data-toggle="tooltip" title="Obligatoire">*</span> Montant :</label>
                        <input type="number" step="any" min="0.1" class="form-control" name="montant" id="montant" value="<?=$charge->montant?>" placeholder="Montant" required>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" name="description" placeholder="Description" maxlength="200" rows="3" required><?=$charge->description?></textarea>
                    </div>
                </div>
...
<script type="text/javascript">
    $(document).ready(function ()
    {
        $("#charge_methode").on("change", function ()
        {
            if($(this).val() != undefined && $(this).val() != null)
            {
                if($(this).val() == 1)
                {
                    ...
                    $("#montant").attr("max", '<?=(getCaisseTotal() + ($charge->methode == 1 ? $charge->montant : 0))?>');
                }
        ...
        $("#charge_methode").val(<?=$charge->methode?>).trigger("change");
        <?php
            if($charge->methode == 2 || $charge->methode == 3) :
                $cheque = getPaiementDetails($charge->id_charge, "charge");
        ?>
