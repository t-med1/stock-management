<form method="post" action="<?=base_url()?>index.php/client_cmd/modifier">
    <div class="row">
        <div class="col-md-12">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="code_client_cmd" id="code_client_cmd" placeholder="Code Commande de Client" required>
                                <span style="color: red; display: none;" id="code_client_cmd_exists"></span>
                                <input type="hidden" name="id_client_cmd" value="<?=$client_cmd->id_client_cmd?>">
                            </div>
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="id_client" id="id_client" required>
                                    <?php
                                        $client_exixsts = false;
                                        foreach ($clients as $val):
                                            if($val["id_client"] == $client_cmd->id_client) {
                                                $client_exixsts = true;
                                            }
                                    ?>
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control" name="date_client_cmd" value="<?=date("Y-m-d", strtotime($client_cmd->date_client_cmd))?>" placeholder="Date de Commande de Client" oninput="getTotal()" required>
                            </div>
                            <div class="form-group">
                                <label>Remarque :</label>
                                <textarea class="form-control" name="remarque" maxlength="200" rows="3" placeholder="Remarque"><?=$client_cmd->remarque?></textarea>
                            </div>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function ()
    {
        ...
        $("#id_client").val('<?=$client_cmd->id_client?>').trigger("change");

        createCodeChecker(
            "client_cmd",
            "CC",
            "<?=$client_cmd->code_client_cmd?>",
            "<?=$client_cmd->id_client_cmd?>"
        );
        ...
        $("form").on("submit", function (event) {
            ...
            if($("[name='id_produit[]']").length > 0) {
                $("#alert_produit").hide();
                checkCodeExists("client_cmd", <?=$client_cmd->id_client_cmd?>, function () {
