...
class Produit extends CI_Controller
{
	...
	public function details($id)
	{
        $data = array(
            ...
            "ventes"        => $this->db->query("SELECT vd.*, v.code_vente, v.num_facture, v.date_vente, v.id_client, cl.full_name AS client FROM vente_details vd LEFT JOIN vente v ON v.id_vente = vd.id_vente LEFT JOIN `client` cl ON cl.id_client = v.id_client WHERE vd.id_produit = ".$id)->result_array(),            "avoirs_in"     => $this->db->query("SELECT ad.*, a.code_avoir, a.date_avoir, a.id_client, c.full_name AS client FROM avoir_details_in ad LEFT JOIN avoir a ON a.id_avoir = ad.id_avoir LEFT JOIN `client` c ON c.id_client = a.id_client WHERE ad.id_produit = ".$id)->result_array(),
    ...
    public function getData($type = null, $table = null, $id_table = null, $details = null) {
        try {
            ...
            if (isset($postData['order'][0])) {
                ...
                $orderBy = $postData['columns'][$order['column']]['data'];
                $orderDir = $order['dir'];
            }
            
            $searchValue = isset($postData['search']['value']) ? $postData['search']['value'] : '';
            
            $this->db->select('p.id_produit, p.full_name, p.image, c.full_name AS category, s.full_name AS sub_category, p.prix_achat, p.prix_vente, p.stock');
            $this->db->from('produit p');
            $this->db->join('categorie c', 'c.id_categorie = p.id_categorie', 'left');
            $this->db->join('sub_categorie s', 's.id_sub_categorie = p.id_sub_categorie', 'left');
            
            if (!is_null($type) && !is_null($table) && !is_null($id_table)) {
                $this->db->where($table . '.' . $id_table, $type);
            }
            
            if (!empty($searchValue)) {
                $this->db->like('p.full_name', $searchValue);
            }
            ...
            foreach ($products as $product) {
                $options = '<a href="' . base_url() . 'edit/' . $product->id_produit . '" class="btn btn-success rounded btn-sm" style="margin-left: 3px;" data-toggle="tooltip" data-placement="top" title="Modifier">' .
                            '<i class="fa fa-wrench" aria-hidden="true"></i>' .
                            '</a>' .
                           '<form method="post" action="' . base_url() . 'delete/' . $product->id_produit . '" id="form_' . $product->id_produit . '" style="display:none;">' .
                           '</form>' .
                           '<a href="#" onclick="confirmDelete(\'form_' . $product->id_produit . '\')" class="btn btn-danger btn-sm rounded" data-toggle="tooltip" data-placement="top" title="Supprimer">' .
            ...
            $response = array(
                "draw" => $draw,
                "recordsTotal" => $this->db->count_all_results('produit'),
                "recordsFiltered" => count($products),
                "data" => $data
            );
            
            // Send JSON response with correct status code
            $this->output->set_status_header(200)->set_content_type('application/json')->set_output(json_encode($response));
        } catch (Exception $e) {
            log_message('error', 'Error in getData(): ' . $e->getMessage());
            // Send JSON response with error message
            $this->output->set_status_header(500)->set_content_type('application/json')->set_output(json_encode(array("error" => "An error occurred while processing the request.")));
        }
    }
