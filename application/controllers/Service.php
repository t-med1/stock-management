    ...
    public function getData($type = null, $table = null, $id_table = null, $details = null)
{
    ...
    $query = "SELECT s.*, c.full_name AS categorie, sc.full_name AS sub_categorie 
              FROM service s 
              LEFT JOIN categorie c ON c.id_categorie = s.id_categorie 
              LEFT JOIN sub_categorie sc ON sc.id_sub_categorie = s.id_sub_categorie 
              WHERE s.display = " . $display . $table_sql . $searchQuery . $orderByColumnQuery . $limitQuery;
    ...
    $response = array(
        ...
        "iTotalDisplayRecords" => count($records), // Use the count of fetched records
        "aaData" => $services
    );
