...
//            redirect("/produit/details/" . $id_produit, "refresh");
    ...
    public function e_modifier($id = null)
    {
        ...
        if (!empty($id_produit_end)) {
            ...
            redirect("/produit/details/" . $this->input->post("id_produit_end"), "refresh");
        } else {
            $data = array(
    ...
    public function getProduits($search = "name", $quantite = null, $cout = null, $type = "ALL")
    {
        ...
        //     if (isset($stock[$produit["id_produit"]])) {
        //         $produit["stock"] = $stock[$produit["id_produit"]];
        //     } else {
        //         $produit["stock"] = 0; // ou une autre valeur par défaut si le stock n'est pas trouvé
        //     }
        //  }
    ...
    public function getData($type = null, $table = null, $id_table = null, $details = null)
    {
        ...
        (SELECT SUM(quantite) FROM production WHERE id_produit = p.id_produit) AS production_quantity
 FROM produit p
 LEFT JOIN categorie c ON c.id_categorie = p.id_categorie
 LEFT JOIN sub_categorie sc ON sc.id_sub_categorie = p.id_sub_categorie
 WHERE p.display = " . $display . $table_sql . $searchQuery . $orderByColumnQuery . $limitQuery;
        ...
        foreach ($records as $record) {
            $quantity = getQuantiteProduit($record["id_produit"]);
            ...
            $recordData = array(
                ...
                //"stock" => $record["stock"],
                "quantite" => getQuantiteProduit($record["id_produit"])["stock"],
